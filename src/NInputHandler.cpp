...
uint NInputHandler::getParticleSizes(const string& particleSizesStr, vector<uint>& particleSizes) {
	...
	while (extract(sizeStrm, currsize)) {
		if (currsize == 0)
			throw NError(RES_INVALID_SIZES);
		particleSizes.push_back(currsize);
		stateSize *= currsize;
		if (stateSize > maxMatrixSize)
			throw NError(RES_STATE_TOO_BIG);
		currsize = 0;
	}
...
void NInputHandler::getMatrix(const char* rows, uint stateSize, MatrixXcd& matrix) {
	...
	while (!matrixStrm.eof()) {
		...
		if (row.find("The") != string::npos)
			break;
		if (row.find("ERROR") != string::npos)
			break;
		if (matrixStrm.fail() && !matrixStrm.eof())
			throw NError(RES_STRING_ERROR);
		if (currRow >= stateSize) {
			if (!row.empty())
				throw NError(RES_MATRIX_TOO_BIG);
			continue;
		}
		uint col = 0;
		...
		while (!rowStrm.eof()) {
			if (col >= stateSize)
				throw NError(RES_ROW_SIZE_MISMATCH);
	...
	if (currRow < stateSize)
		throw NError(RES_MATRIX_TOO_SMALL);
}

double NInputHandler::getTargetDistance(const string& targetDistanceStr) {
	if (targetDistanceStr.empty())
		return 0;
	double targetDistance = 0;
	if (!convert(targetDistanceStr, targetDistance))
		throw NError(RES_INVALID_DISTANCE);
	if (targetDistance < 0)
		throw NError(RES_INVALID_DISTANCE);
...
double NInputHandler::getMinProbForState(const string& minProbForStateStr) {
	if (minProbForStateStr.empty())
		return 0;
	double minProbForState = 0;
	if (!convert(minProbForStateStr, minProbForState))
		throw NError(RES_INVALID_PROB);
	if (minProbForState < 0 || minProbForState > 1)
		throw NError(RES_INVALID_PROB);
	return minProbForState;
}

uint NInputHandler::getTargetNumOfStates(const string& targetNumberOfStatesStr) {
	if (targetNumberOfStatesStr.empty())
		return 0;
	uint targetNumberOfStates = 0;
	if (!convert(targetNumberOfStatesStr, targetNumberOfStates))
		throw NError(RES_INVALID_NUM_OF_STATES);
	if (targetNumberOfStates < 1)
		throw NError(RES_INVALID_NUM_OF_STATES);
	return targetNumberOfStates;
}

uint NInputHandler::getOutputPrecision(const string& outputPrecisionStr) {
	if (outputPrecisionStr.empty())
		throw NError(RES_INVALID_PRECISION);
	uint outputPrecision;
	if (!convert(outputPrecisionStr, outputPrecision))
		throw NError(RES_INVALID_PRECISION);
...
void NInputHandler::extract(stringstream& strm, ValType& val) {
	...
	size_t secondDelim = (firstDelim != std::string::npos) ? std::min(valStr.substr(firstDelim + 1).find('-'),
	                                                                  valStr.substr(firstDelim + 1).find('+'))
	                                                       : std::string::npos;
	// No i:
	if (iDelim == string::npos) {
		...
		if ((firstDelim == std::string::npos) || ((firstDelim == 0) && (secondDelim == std::string::npos))) {
